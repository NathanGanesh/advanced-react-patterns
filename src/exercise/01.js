...
function updateUser(dispatch, oekdeal, updates) {
	dispatch({ type: 'start update', updates });
	return userClient
		.updateUser(oekdeal, updates)
		.then(
...
function UserSettings() {
	...
	function handleSubmit(event) {
		...
		updateUser(userDispatch, user, formState).catch(console.log('oek'));
	}
	...
	return (
		<form onSubmit={handleSubmit}>
			...
			<div style={{ marginBottom: 12 }}>
				...
				<textarea id="bio" name="bio" value={formState.bio} onChange={handleChange} style={{ width: '100%' }} />
			</div>
			<div>
				<button
					type="button"
